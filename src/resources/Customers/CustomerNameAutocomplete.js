...
class CustomerNameAutocomplete extends React.Component {
  handleSuggestionSelected = (event, {suggestion, method}) => {
    this.props.updateCustInfo({address: suggestion.streetAddress, city: suggestion.city, state: suggestion.state, zipCode: suggestion.zipCode, phone: suggestion.phone, custEmail: suggestion.custEmail});
  };
  render () {
    const {updateCustInfo, ...rest} = this.props;
    return (
      <ReferenceInput label='Customer Name' source='customerName' reference='customers'
        filterToQuery={searchText => ({customer_name: searchText}) } {...rest}
      >
